...
GDT.addPlatform({
    ...
    events: [{
        ...
        getNotification: function (company) {
            return new Notification({
				header : "Industry News".localize(),
                    text : "Today Ninvento released the successor to the Gameling, the Gameling Spectrum. {n} This new handheld device features an all new colour display, a slightly redesigned look to it, and is overall smaller than the original Gameling. {n} This product sure is expected to do brilliant in the handheld market. {0}.".localize().format(General.getETADescription('10/10/2', '10/11/3')),
                    image : Placeholder_Icon,
					buttonText : ["Next", "Next", "OK"],
            });
        }
...
GDT.addPlatform({
    ...
    events: [{
        ...
        getNotification: function (company) {
            return new Notification({
				header : "Industry News".localize(),
                    text : "A new handheld device by Ninvento was announced today as the successor to the Gameling Spectrum, the Gameling Retreat. {n} The Gameling Retreat is an improvement in a couple of ways.First of all this device has a backlit display so that you can better see what you're doing, and also has a built in rechargable battery so you're not always replacing them when they run out. {n} Additionally this device comes in a few forms so you can choose a model that better suits you. {0}.".localize().format(General.getETADescription('13/5/3', '13/6/2')),
                    image : Placeholder_Icon,
					buttonText : ["Next", "Next", "OK"],
            });
        }
...
GDT.addPlatform({
    ...
    events: [{
        ...
        getNotification: function (company) {
            return new Notification({
				header : "Industry News".localize(),
                    text : "Ninvento has today announced the successor to the Ninvento GS, the Ninvento GS Slim. {n} As the name suggests this is a slimer, lighter and more lightweight version of the original GS. {n} The demand for the original Ninvento GS is expected to be the same with this one. {0}.".localize().format(General.getETADescription('13/5/3', '13/6/2')),
                    image : Placeholder_Icon,
					buttonText : ["Next", "Next", "OK"],
            });
        }
...
GDT.addPlatform({
    ...
    events: [{
        ...
        getNotification: function (company) {
            return new Notification({
				header : "Industry News".localize(),
                    text : "After many years since the first one, Ninvento has released a new GS3D, this one named the GS3D+. {n} This model has an improved overall design and much more powerful internals so it can run better games with better graphics and more content, all in a handheld device. {n} With the low price tag this platform should do relatively well, just like the original GS3D. {0}.".localize().format(General.getETADescription('13/5/3', '13/6/2')),
                    image : Placeholder_Icon,
					buttonText : ["Next", "Next", "OK"],
            });
        }
...
GDT.addPlatform({
    ...
    events: [{
        ...
        getNotification: function (company) {
            return new Notification({
				header : "Industry News".localize(),
                    text : "Ninvento has today released a new product to the public named the Ninvento Twist. {n} What makes this console special is that it can be both used as a home gaming console and taken around with you by simply charging it up and taking it with you, the player can use the joy cons on the side of the device when they're out and then connect them together and use them as a wireless controller when they're at home. {n} This device should do fairly well and there are many titles expected to be released for this product. {0}.".localize().format(General.getETADescription('29/2/2', '29/3/1')),
                    image : Placeholder_Icon,
					buttonText : ["Next", "Next", "OK"],
            });
        }
